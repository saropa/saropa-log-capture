...
const viewer_content_1 = require("../../ui/provider/viewer-content");
suite('Viewer HTML', () => {
    suite('getEffectiveViewerLines', () => {
        test('returns maxLines when viewerMaxLines is 0', () => {
            assert.strictEqual((0, viewer_content_1.getEffectiveViewerLines)(100_000, 0), 100_000);
            assert.strictEqual((0, viewer_content_1.getEffectiveViewerLines)(30_000, 0), 30_000);
        });
        test('returns min(viewerMaxLines, maxLines) when viewerMaxLines > 0', () => {
            assert.strictEqual((0, viewer_content_1.getEffectiveViewerLines)(100_000, 25_000), 25_000);
            assert.strictEqual((0, viewer_content_1.getEffectiveViewerLines)(100_000, 80_000), 80_000);
        });
        test('caps at maxLines when viewerMaxLines exceeds maxLines', () => {
            assert.strictEqual((0, viewer_content_1.getEffectiveViewerLines)(20_000, 50_000), 20_000);
            assert.strictEqual((0, viewer_content_1.getEffectiveViewerLines)(10_000, 100_000), 10_000);
        });
        test('handles viewerMaxLines equal to maxLines', () => {
            assert.strictEqual((0, viewer_content_1.getEffectiveViewerLines)(50_000, 50_000), 50_000);
        });
    });
    suite('buildViewerHtml viewerMaxLines', () => {
        test('injects viewerMaxLines into viewer script when provided', () => {
            const html = (0, viewer_content_1.buildViewerHtml)({ nonce: (0, viewer_content_1.getNonce)(), version: '0.0.0', viewerMaxLines: 25_000 });
    ...
    suite('codicon stylesheet', () => {
        test('should include codicon link tag when URI provided', () => {
            ...
            const html = (0, viewer_content_1.buildViewerHtml)({ nonce: (0, viewer_content_1.getNonce)(), version: '0.0.0', cspSource: 'https://file+.vscode-resource.test', codiconCssUri: uri });
        ...
        test('should add cspSource to style-src when provided', () => {
            ...
            const html = (0, viewer_content_1.buildViewerHtml)({ nonce: (0, viewer_content_1.getNonce)(), version: '0.0.0', cspSource: csp });
    ...
    suite('script syntax', () => {
        test('should define stripTags null-safe so Copy All never throws on missing line.html', () => {
            const html = (0, viewer_content_1.buildViewerHtml)({ nonce: (0, viewer_content_1.getNonce)(), extensionUri: 'https://example.com', version: '0.0.0' });
        ...
        test('should produce HTML with no script syntax errors', () => {
            ...
            while ((match = scriptRegex.exec(html)) !== null) {
                ...
                try {
                    ...
                    const err = e;
                    assert.fail(`SyntaxError in script block ${blockIndex}: ${err.message}`);
                }
                blockIndex++;
            }
...
//# sourceMappingURL=viewer-script-syntax.test.js.map