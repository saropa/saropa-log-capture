...
suite('Webview element ID wiring', () => {
    ...
    function extractScriptGetElementByIdCalls(html) {
    ...
    test('every getElementById in scripts references an element in the HTML', () => {
        ...
        assert.deepStrictEqual(missing, [], `getElementById references to non-existent HTML elements:\n  ${missing.join('\n  ')}\n\nEither add the id to the HTML, update the script, or add to the allowlist with a comment.`);
    });
    test('allowlist entries are still needed (no stale allowlist)', () => {
        ...
        assert.deepStrictEqual(staleEntries, [], `Allowlist has stale entries:\n  ${staleEntries.join('\n  ')}`);
    });
});
//# sourceMappingURL=viewer-element-wiring.test.js.map