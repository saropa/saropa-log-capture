...
import { getFilterDrawerStyles } from '../../ui/viewer-styles/viewer-styles-filter-drawer';
...
suite('Viewer toolbar animations', () => {
    ...
    suite('toolbar styles CSS', () => {
        ...
        test('should set transform-origin: top on actions popover', () => {
            assert.match(
                css,
                /\.toolbar-actions-popover\s*\{[^}]*transform-origin:\s*top/s,
                'actions popover needs transform-origin: top for scaleY animation',
            );
        });
        ...
        test('reduced-motion media query should cover all animation classes', () => {
            assert.match(
                css,
                /prefers-reduced-motion.*anim-flyout-open/s,
                'reduced motion must cover anim-flyout-open',
            );
            assert.match(
                css,
                /prefers-reduced-motion.*anim-dropdown-close/s,
                'reduced motion must cover anim-dropdown-close',
            );
        });
    });
    ...
    suite('Signals styles CSS', () => {
        let css: string;
        setup(() => { css = getRootCauseHypothesesStyles(); });

        test('should define signals-drawer-hidden collapse class', () => {
            assert.match(
                css,
                /\.root-cause-hypotheses\.signals-drawer-hidden\s*\{/,
                'signals-drawer-hidden class must exist',
            );
        });
        ...
        test('signals-drawer-hidden should set max-height: 0 and opacity: 0', () => {
            assert.match(
                css,
                /signals-drawer-hidden\s*\{[^}]*max-height:\s*0/s,
                'must collapse height',
            );
            assert.match(
                css,
                /signals-drawer-hidden\s*\{[^}]*opacity:\s*0/s,
                'must fade out',
            );
        });
        ...
        test('should have transition on root-cause-hypotheses for smooth collapse', () => {
            assert.match(
                css,
                /\.root-cause-hypotheses\s*\{[^}]*transition:[^}]*max-height/s,
                'signals needs max-height transition',
            );
        });
        ...
        test('reduced-motion should disable signals transitions', () => {
            assert.match(
                css,
                /prefers-reduced-motion.*\.root-cause-hypotheses/s,
                'reduced motion must cover signals transitions',
            );
        });
    });
    ...
    suite('toolbar script animation wiring', () => {
        ...
        test('should check prefers-reduced-motion', () => {
            assert.ok(
                ...
                'script must check reduced motion preference',
            );
        });
        ...
        test('actions popover should have animationend handler for toolbar-actions-open', () => {
            assert.ok(
                src.includes("remove('toolbar-actions-open'"),
                'actions close animation must remove toolbar-actions-open on end',
            );
        });
    });
