...
suite('viewer-sql-query-history panel HTML', () => {
    ...
    test('duration header uses abbreviated label with tooltip', () => {
        ...
        assert.ok(html.includes('title="Slowest duration in milliseconds"'), 'header should have full description as tooltip');
        assert.ok(!html.includes('>Slowest (ms)<'), 'old verbose header label should be removed');
    });
