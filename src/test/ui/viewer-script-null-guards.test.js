...
const viewer_goto_line_1 = require("../../ui/viewer/viewer-goto-line");
const viewer_copy_1 = require("../../ui/viewer/viewer-copy");
const viewer_search_1 = require("../../ui/viewer-search-filter/viewer-search");
const viewer_hidden_lines_1 = require("../../ui/viewer/viewer-hidden-lines");
const viewer_auto_hide_modal_1 = require("../../ui/viewer/viewer-auto-hide-modal");
const viewer_data_viewport_1 = require("../../ui/viewer/viewer-data-viewport");
const viewer_replay_1 = require("../../ui/viewer/viewer-replay");
const viewer_session_nav_1 = require("../../ui/viewer-nav/viewer-session-nav");
const viewer_split_nav_1 = require("../../ui/viewer-nav/viewer-split-nav");
suite('Webview script null guards', () => {
    ...
    suite('viewer-goto-line', () => {
        const script = (0, viewer_goto_line_1.getGotoLineScript)();
        test('should guard logEl.scrollTop in openGotoLine', () => {
            const block = script.slice(script.indexOf('function openGotoLine'), script.indexOf('function openGotoLine') + 200);
            assert.ok(block.includes('logEl ? logEl.scrollTop : 0'), 'openGotoLine should guard logEl.scrollTop');
        });
        test('should guard logEl in closeGotoLine', () => {
            const block = script.slice(script.indexOf('function closeGotoLine'), script.indexOf('function closeGotoLine') + 200);
            assert.ok(block.includes('&& logEl'), 'closeGotoLine should guard logEl');
        });
        test('should guard logEl in scrollToLineNumber', () => {
            const block = script.slice(script.indexOf('function scrollToLineNumber'), script.indexOf('function scrollToLineNumber') + 300);
            assert.ok(block.includes('if (!logEl) return'), 'scrollToLineNumber should early-return when logEl is null');
        });
        test('should guard jumpBtn.style in scrollToLineNumber', () => {
            const block = script.slice(script.indexOf('function scrollToLineNumber'), script.indexOf('function scrollToLineNumber') + 500);
            assert.ok(block.includes('if (jumpBtn) jumpBtn.style'), 'scrollToLineNumber should guard jumpBtn.style');
        });
    });
    suite('viewer-copy', () => {
        const script = (0, viewer_copy_1.getCopyScript)();
        test('should guard copyFloat/wrapperEl/logEl in showCopyFloat', () => {
            const block = script.slice(script.indexOf('function showCopyFloat'), script.indexOf('function showCopyFloat') + 200);
            assert.ok(block.includes('if (!copyFloat || !wrapperEl || !logEl) return'), 'showCopyFloat should guard all three elements');
        });
        test('should guard copyFloat in hideCopyFloat', () => {
            const block = script.slice(script.indexOf('function hideCopyFloat'), script.indexOf('function hideCopyFloat') + 100);
            assert.ok(block.includes('if (!copyFloat) return'), 'hideCopyFloat should guard copyFloat');
        });
        test('should guard viewportEl in updateSelectionHighlight', () => {
            const block = script.slice(script.indexOf('function updateSelectionHighlight'), script.indexOf('function updateSelectionHighlight') + 150);
            assert.ok(block.includes('if (!viewportEl) return'), 'updateSelectionHighlight should guard viewportEl');
        });
        test('should guard viewportEl in clearSelection', () => {
            const block = script.slice(script.indexOf('function clearSelection'), script.indexOf('function clearSelection') + 150);
            assert.ok(block.includes('if (!viewportEl) return'), 'clearSelection should guard viewportEl');
        });
        test('should guard viewportEl.addEventListener for mouseover', () => {
            assert.ok(script.includes("if (viewportEl) viewportEl.addEventListener('mouseover'"), 'mouseover listener should guard viewportEl');
        });
        test('should guard copyFloat.addEventListener for click', () => {
            assert.ok(script.includes("if (copyFloat) copyFloat.addEventListener('click'"), 'click listener should guard copyFloat');
        });
        test('should guard logEl.addEventListener for scroll', () => {
            assert.ok(script.includes("if (logEl) logEl.addEventListener('scroll'"), 'scroll listener should guard logEl');
        });
    ...
    suite('viewer-search', () => {
        const script = (0, viewer_search_1.getSearchScript)();
        test('should guard searchInputEl in openSearch', () => {
            const block = script.slice(script.indexOf('function openSearch'), script.indexOf('function openSearch') + 150);
            assert.ok(block.includes('if (!searchInputEl) return'), 'openSearch should early-return when searchInputEl is null');
        });
        test('should guard matchCountEl in clearSearchState', () => {
            const block = script.slice(script.indexOf('function clearSearchState'), script.indexOf('function clearSearchState') + 200);
            assert.ok(block.includes("if (matchCountEl) matchCountEl.textContent"), 'clearSearchState should guard matchCountEl');
        });
        test('should guard searchInputEl.value in updateSearch', () => {
            const block = script.slice(script.indexOf('function updateSearch'), script.indexOf('function updateSearch') + 200);
            assert.ok(block.includes("searchInputEl ? searchInputEl.value : ''"), 'updateSearch should guard searchInputEl.value');
        });
        test('should guard matchCountEl in updateMatchDisplay', () => {
            const block = script.slice(script.indexOf('function updateMatchDisplay'), script.indexOf('function updateMatchDisplay') + 300);
        ...
        test('should guard searchInputEl.addEventListener for focus', () => {
            assert.ok(script.includes("if (searchInputEl) searchInputEl.addEventListener('focus'"), 'focus listener should guard searchInputEl');
        });
        test('should guard searchInputEl.addEventListener for keydown', () => {
            assert.ok(script.includes("if (searchInputEl) searchInputEl.addEventListener('keydown'"), 'keydown listener should guard searchInputEl');
        });
        test('should guard search-next/search-prev addEventListener', () => {
            assert.ok(script.includes('if (searchNextBtn) searchNextBtn.addEventListener'), 'search-next listener should guard element');
            assert.ok(script.includes('if (searchPrevBtn) searchPrevBtn.addEventListener'), 'search-prev listener should guard element');
        });
    });
    suite('viewer-hidden-lines', () => {
        const script = (0, viewer_hidden_lines_1.getHiddenLinesScript)();
        test('should guard querySelector for hidden-count-text', () => {
            const block = script.slice(script.indexOf('function updateHiddenDisplay'), script.indexOf('function updateHiddenDisplay') + 500);
            assert.ok(block.includes("if (countTextEl) countTextEl.textContent"), 'updateHiddenDisplay should guard querySelector result');
        });
    });
    suite('viewer-auto-hide-modal', () => {
        const script = (0, viewer_auto_hide_modal_1.getAutoHideModalScript)();
        test('should guard querySelector for backdrop in initAutoHideModal', () => {
            const block = script.slice(script.indexOf('function initAutoHideModal'), script.indexOf('function initAutoHideModal') + 400);
            assert.ok(block.includes("if (backdropEl) backdropEl.addEventListener"), 'backdrop listener should guard querySelector result');
        });
        test('should guard querySelector for close btn in initAutoHideModal', () => {
            const block = script.slice(script.indexOf('function initAutoHideModal'), script.indexOf('function initAutoHideModal') + 600);
            assert.ok(block.includes("if (closeEl) closeEl.addEventListener"), 'close button listener should guard querySelector result');
        });
        test('should guard querySelector for list in initAutoHideModal', () => {
            const block = script.slice(script.indexOf('function initAutoHideModal'), script.indexOf('function initAutoHideModal') + 600);
            assert.ok(block.includes("if (listEl) listEl.addEventListener"), 'list listener should guard querySelector result');
        });
        test('should guard list.innerHTML in populateAutoHideModal', () => {
            const block = script.slice(script.indexOf('function populateAutoHideModal'), script.indexOf('function populateAutoHideModal') + 600);
            assert.ok(block.includes("if (list) list.innerHTML"), 'populateAutoHideModal should guard list.innerHTML');
        });
    });
    ...
    suite('viewer-replay', () => {
        ...
        test('should guard footerActionsMenu in click handler', () => {
            assert.ok(script.includes('footerActionsMenu && footerActionsMenu.classList.contains'), 'footerActionsBtn click should guard footerActionsMenu');
        });
    });
    suite('viewer-session-nav', () => {
        const script = (0, viewer_session_nav_1.getSessionNavScript)();
        test('should guard sessionNav.classList in updateSessionNav', () => {
            assert.ok(script.includes("if (sessionNav) sessionNav.classList"), 'updateSessionNav should guard sessionNav');
        });
        test('should guard sessionPrevBtn.addEventListener', () => {
            assert.ok(script.includes("if (sessionPrevBtn) sessionPrevBtn.addEventListener"), 'sessionPrevBtn listener should be guarded');
        });
        test('should guard sessionNextBtn.addEventListener', () => {
            assert.ok(script.includes("if (sessionNextBtn) sessionNextBtn.addEventListener"), 'sessionNextBtn listener should be guarded');
        });
    });
    suite('viewer-split-nav', () => {
        const script = (0, viewer_split_nav_1.getSplitNavScript)();
        test('should guard splitBreadcrumb.classList in updateSplitInfo', () => {
            assert.ok(script.includes("if (splitBreadcrumb) splitBreadcrumb.classList"), 'updateSplitInfo should guard splitBreadcrumb');
        });
        test('should guard splitPrevBtn.addEventListener', () => {
            assert.ok(script.includes("if (splitPrevBtn) splitPrevBtn.addEventListener"), 'splitPrevBtn listener should be guarded');
        });
        test('should guard splitNextBtn.addEventListener', () => {
            assert.ok(script.includes("if (splitNextBtn) splitNextBtn.addEventListener"), 'splitNextBtn listener should be guarded');
        });
    });
    ...
    suite('error handler banner', () => {
        ...
        test('should use addEventListener for error events', () => {
            assert.ok(script.includes("window.addEventListener('error'"), 'should use addEventListener instead of window.onerror');
        });
        test('should capture stack trace from error event', () => {
            assert.ok(script.includes('ev.error.stack'), 'should extract stack from error event');
        });
        test('should include line and col in error text', () => {
            assert.ok(script.includes("line + ', col ' + col"), 'banner should show line and column numbers');
        });
        test('should include a Copy button in the banner', () => {
            assert.ok(script.includes("copyBtn.textContent = 'Copy'"), 'banner should have a copy button');
        });
        test('should make error text selectable', () => {
            assert.ok(script.includes('user-select:text'), 'banner text should be user-selectable');
        });
    });
});
