    ...
    suite('getContextMenuHtml', () => {
        ...
        test('should use context-menu-label class on all toggle text spans', () => {
            ...
            for (const action of toggleActions) {
                const pattern = new RegExp(`data-action="${action}"[\\s\\S]{0,300}class="context-menu-label"`);
                assert.ok(pattern.test(html), `toggle ${action} should have context-menu-label on its text span`);
            }
        });
    });
