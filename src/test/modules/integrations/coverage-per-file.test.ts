...
import {
    parseLcovPerFile,
    parseCoberturaPerFile,
    parseSummaryJsonPerFile,
    lookupCoverage,
} from '../../../modules/integrations/providers/coverage-per-file';
...
suite('CoveragePerFile', () => {
    ...
    test('should parse lcov with multiple files', () => {
        ...
        const map = parseLcovPerFile(lcov);
    ...
    test('should parse cobertura with multiple classes', () => {
        ...
        const map = parseCoberturaPerFile(xml);
        assert.strictEqual(map.size, 2);
    ...
    test('should parse Istanbul summary JSON per file', () => {
        ...
        const map = parseSummaryJsonPerFile(json);
        assert.strictEqual(map.size, 2);
    ...
    test('should find exact normalized match', () => {
        ...
        assert.strictEqual(lookupCoverage(map, 'src/foo.ts'), 85);
    });
    ...
    test('should fall back to basename match', () => {
        ...
        assert.strictEqual(lookupCoverage(map, 'completely/different/foo.ts'), 42);
    });

    test('should return undefined when no match', () => {
        const map = new Map([['src/foo.ts', 85]]);
        assert.strictEqual(lookupCoverage(map, 'src/baz.ts'), undefined);
    });

    test('should handle empty map', () => {
        const map = new Map<string, number>();
        assert.strictEqual(lookupCoverage(map, 'anything.ts'), undefined);
    });
    ...
    test('should return undefined for ambiguous basename match', () => {
        ...
        assert.strictEqual(lookupCoverage(map, 'other/index.ts'), undefined);
    });
    ...
    test('should return empty map for invalid JSON', () => {
        const map = parseSummaryJsonPerFile('not json at all');
        assert.strictEqual(map.size, 0);
    });
    ...
    test('should lookup coverage at boundary values', () => {
        ...
        assert.strictEqual(lookupCoverage(map, 'src/a.ts'), 80);
        assert.strictEqual(lookupCoverage(map, 'src/b.ts'), 50);
        assert.strictEqual(lookupCoverage(map, 'src/c.ts'), 49);
        assert.strictEqual(lookupCoverage(map, 'src/d.ts'), 0);
        assert.strictEqual(lookupCoverage(map, 'src/e.ts'), 100);
    });
    ...
    test('should handle Windows backslash paths in lookup', () => {
        ...
        assert.strictEqual(lookupCoverage(map, 'src\\foo.ts'), 75);
    });
});
