...
const health_score_1 = require("../../../modules/misc/health-score");
suite('health-score', () => {
    suite('getHealthScoreParams', () => {
        test('should return built-in impactWeights and decayRate when extension absent', () => {
            const params = (0, health_score_1.getHealthScoreParams)();
    ...
    suite('parseConsumerContractHealthScoreParams', () => {
        test('should parse healthScore from consumer contract JSON', () => {
            const parsed = (0, health_score_1.parseConsumerContractHealthScoreParams)({
                healthScore: {
    ...
    suite('computeHealthScore', () => {
        test('should return ~94 for 10 medium violations across 50 files', () => {
            const result = (0, health_score_1.computeHealthScore)({ medium: 10 }, 50);
            assert.ok(result);
            assert.strictEqual(result.score, 94);
        });
        test('should return low score for critical-heavy project', () => {
            const result = (0, health_score_1.computeHealthScore)({ critical: 10, high: 20 }, 50);
            assert.ok(result);
            // (10*8 + 20*3) / 50 = 2.8, exp(-0.84) ≈ 0.432 → 43
            assert.strictEqual(result.score, 43);
        });
        test('should return undefined when filesAnalyzed is 0', () => {
            assert.strictEqual((0, health_score_1.computeHealthScore)({ critical: 5 }, 0), undefined);
        });
        test('should return score 100 when byImpact is empty', () => {
            const result = (0, health_score_1.computeHealthScore)({}, 50);
            assert.ok(result);
            assert.strictEqual(result.score, 100);
            assert.strictEqual(result.weightedViolations, 0);
        });
        test('should handle all impact levels', () => {
            const result = (0, health_score_1.computeHealthScore)({ critical: 1, high: 2, medium: 5, low: 10, opinionated: 20 }, 100);
            assert.ok(result);
        ...
        test('should ignore non-numeric values in byImpact', () => {
            const byImpact = { critical: 5 };
            // Simulate bad data by casting
            byImpact['high'] = 'bad';
            const result = (0, health_score_1.computeHealthScore)(byImpact, 10);
            assert.ok(result);
        ...
        test('should match saropa_lints constants', () => {
            ...
            const result = (0, health_score_1.computeHealthScore)({ critical: 3, high: 12, medium: 45, low: 72, opinionated: 10 }, 247);
            assert.ok(result);
        ...
        test('should use explicit params when provided', () => {
            ...
            const result = (0, health_score_1.computeHealthScore)({ critical: 2, high: 4 }, 100, customParams);
            assert.ok(result);
    ...
    suite('formatHealthScoreLine', () => {
        test('should produce correct markdown with tier and count', () => {
            const line = (0, health_score_1.formatHealthScoreLine)({
                byImpact: { medium: 10 },
                filesAnalyzed: 50,
                tier: 'comprehensive',
                totalViolations: 10,
            });
            assert.strictEqual(line, '**Project health: 94/100** (comprehensive tier, 10 violations)');
        });
        test('should return undefined when filesAnalyzed is 0', () => {
            assert.strictEqual((0, health_score_1.formatHealthScoreLine)({ byImpact: {}, filesAnalyzed: 0, tier: 'comprehensive', totalViolations: 0 }), undefined);
        });
    });
    suite('formatHealthScoreBreakdown', () => {
        test('should list only non-zero impact levels', () => {
            const result = (0, health_score_1.formatHealthScoreBreakdown)({ critical: 3, high: 0, medium: 12, low: 0, opinionated: 5 });
            assert.strictEqual(result, '3 critical, 12 medium, 5 opinionated');
        });
        test('should return undefined when all counts are zero', () => {
            assert.strictEqual((0, health_score_1.formatHealthScoreBreakdown)({ critical: 0, high: 0 }), undefined);
        });
        test('should return undefined for empty byImpact', () => {
            assert.strictEqual((0, health_score_1.formatHealthScoreBreakdown)({}), undefined);
        });
    });
});
