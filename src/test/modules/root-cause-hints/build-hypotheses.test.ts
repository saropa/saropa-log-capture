...
test("buildHypotheses: high-confidence ANR absorbs error-recent evidence lines", () => {
  const hy = buildHypotheses({
    ...base,
    errors: [
  ...
  assert.ok(anr.evidenceLineIds.includes(10), "ANR should carry error line 10");
  assert.ok(anr.evidenceLineIds.includes(11), "ANR should carry error line 11");
...
test("buildHypotheses: high ANR merge does not affect non-error hypothesis types", () => {
  ...
  const anr = hy.find((h) => h.templateId === "anr-risk");
  assert.ok(anr, "ANR hypothesis should be present");
  assert.ok(anr.evidenceLineIds.includes(5), "ANR should carry merged error line 5");
  assert.strictEqual(hy.filter((h) => h.templateId === "error-recent").length, 0);
  assert.ok(hy.some((h) => h.templateId === "fingerprint-leader"));
});
