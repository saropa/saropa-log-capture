...
suite("drift-log-line-args-fold", () => {
    ...
    test("buildDriftArgsFoldHtml encodes suffix in title and body", () => {
        const h = (0, drift_log_line_args_fold_1.buildDriftArgsFoldHtml)(" with args []");
    ...
    test("should render suffix exactly once, inside fold wrapper only", () => {
        ...
        assert.strictEqual(afterFold.includes("with args"), false, "suffix must not appear after fold wrapper (was rendered twice before fix)");
    });
    test("should render non-empty args inside fold wrapper", () => {
        ...
        const foldEnd = html.indexOf('</span></span>');
        const afterFold = html.substring(foldEnd + '</span></span>'.length);
        assert.strictEqual(afterFold.includes("with args"), false, "non-empty args suffix must not appear after fold wrapper");
    });
    test("non-Drift lines match plain ansiLinkify path", () => {
        const plain = "hello world";
        assert.strictEqual((0, drift_log_line_args_fold_1.buildLogLineHtmlWithOptionalDriftArgsFold)(plain), (0, drift_log_line_args_fold_1.ansiLinkifyLineHtml)(plain));
    });
});
