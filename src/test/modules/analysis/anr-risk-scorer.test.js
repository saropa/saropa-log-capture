...
const anr_risk_scorer_1 = require("../../../modules/analysis/anr-risk-scorer");
suite('AnrRiskScorer', () => {
    test('should return score 0 for clean session', () => {
        const result = (0, anr_risk_scorer_1.scanAnrRisk)('hello world\nstarting server\nall good');
    ...
    test('should return score 0 for empty input', () => {
        const result = (0, anr_risk_scorer_1.scanAnrRisk)('');
        assert.strictEqual(result.score, 0);
        assert.strictEqual(result.level, 'low');
    });
    test('should detect ANR keyword and score medium', () => {
        const result = (0, anr_risk_scorer_1.scanAnrRisk)('Application Not Responding in com.example');
    ...
    test('should compute composite score from multiple signal types', () => {
        ...
        const result = (0, anr_risk_scorer_1.scanAnrRisk)(body);
    ...
    test('should be case-insensitive', () => {
        const result = (0, anr_risk_scorer_1.scanAnrRisk)('CHOREOGRAPHER warning\nGC PAUSE detected');
        assert.ok(result.score > 0);
        assert.ok(result.signals.length >= 2);
    });
    test('should handle input dispatching timed out pattern', () => {
        const result = (0, anr_risk_scorer_1.scanAnrRisk)('Input dispatching timed out (Waiting because no window)');
        assert.strictEqual(result.score, 25);
        assert.ok(result.signals.some(s => s.includes('ANR keyword')));
    });
});
//# sourceMappingURL=anr-risk-scorer.test.js.map