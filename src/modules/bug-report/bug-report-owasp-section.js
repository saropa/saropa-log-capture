...
exports.buildOwaspSummaries = buildOwaspSummaries;
exports.formatOwaspSection = formatOwaspSection;
exports.countOwaspCategories = countOwaspCategories;
...
function buildOwaspSummaries(matches) {
...
function formatOwaspSection(matches, primaryFile) {
    const summaries = buildOwaspSummaries(matches);
    if (summaries.length === 0) {
        return undefined;
    }
    const total = summaries.reduce((s, c) => s + c.count, 0);
    const lines = ['## Security Context'];
    if (primaryFile) {
        const primaryOwaspCount = matches.filter(v => v.file === primaryFile && hasOwasp(v)).length;
        if (primaryOwaspCount > 0) {
            const basename = primaryFile.split('/').pop() ?? primaryFile;
            lines.push(`Crash file \`${basename}\` has ${primaryOwaspCount} ` +
                `OWASP-mapped violation${primaryOwaspCount === 1 ? '' : 's'}:`);
        }
        else {
            lines.push(`${total} OWASP-mapped violation${total === 1 ? '' : 's'} ` +
    ...
    const tableRows = summaries.map(s => `| **${s.label}** | ${s.count} | ${s.rules.join(', ')} |`);
    lines.push('| Category | Count | Rules |\n|----------|-------|-------|\n' +
        tableRows.join('\n'));
    lines.push('> These violations may be related to the crash. ' +
        'Review the rules above for potential root causes.');
    return lines.join('\n\n');
}
/** Count OWASP violations and collect unique category IDs for scoring. */
function countOwaspCategories(matches) {
    const ids = new Set();
    let count = 0;
    for (const v of matches) {
        if (!hasOwasp(v)) {
        ...
        count++;
        for (const c of v.owasp.mobile) {
            ids.add(c.toUpperCase());
        }
        for (const c of v.owasp.web) {
            ids.add(c.toUpperCase());
        }
    }
    return { owaspViolationCount: count, owaspCategories: [...ids].sort().join(', ') };
}
//# sourceMappingURL=bug-report-owasp-section.js.map