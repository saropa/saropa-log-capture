...
export function collectTokens(text: string, into: Set<string>, cap: number): void {
...
export function addNormalizedToken(token: string, into: Set<string>, cap: number): void {
    if (into.size >= cap) { return; }
    const normalized = normalizeToken(token);
    if (normalized) { into.add(normalized); }
}

export function stripCommentPreservingQuotes(line: string): string {
    let inSingle = false;
    let inDouble = false;
...
export function normalizeQuotedValue(raw: string): string {
