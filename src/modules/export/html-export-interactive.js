...
exports.exportToInteractiveHtml = exportToInteractiveHtml;
const vscode = __importStar(require("vscode"));
const ansi_1 = require("../capture/ansi");
const session_metadata_1 = require("../session/session-metadata");
const html_export_styles_1 = require("./html-export-styles");
const html_export_script_1 = require("./html-export-script");
const html_export_json_1 = require("./html-export-json");
/**
 ...
 */
async function exportToInteractiveHtml(logUri) {
 ...
 */
function buildInteractiveBody(lines, annotations) {
    const parts = [];
    let groupId = 0;
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (line.isStackFrame) {
            ...
            const jsonHtml = (0, html_export_json_1.wrapJsonInLine)(line.html);
            parts.push(`<div class="${cls}" data-idx="${i}" data-cat="${line.category}">${jsonHtml}</div>`);
        }
        const ann = annotations.get(i);
        if (ann) {
            parts.push(`<div class="annotation">[Note: ${(0, ansi_1.escapeHtml)(ann)}]</div>`);
        }
    }
    if (inStackGroup) {
        parts.push('</div>');
...
function buildInteractiveHtmlDocument(headerHtml, bodyHtml, categories) {
...
<style>
${(0, html_export_styles_1.getInteractiveStyles)()}
</style>
</head>
...
<script>
${(0, html_export_script_1.getInteractiveScript)()}
</script>
</body>
</html>`;
}
//# sourceMappingURL=html-export-interactive.js.map