...
import {
  ExclusionRule,
  parseExclusionPattern,
  testExclusion,
} from "./exclusion-matcher";
import {
  generateSummary,
...
export class SessionManagerImpl implements SessionManager {
  ...
  async startSession(
    ...
    if (session.parentSession && this.sessions.has(session.parentSession.id)) {
      this.sessions.set(
        session.id,
        this.sessions.get(session.parentSession.id)!,
      );
      this.outputChannel.appendLine(
        `Child session aliased to parent: ${session.type}`,
      );
      return;
    }
    ...
    if (!workspaceFolder) {
      this.outputChannel.appendLine(
        "No workspace folder found. Skipping capture.",
      );
      return;
    }

    checkGitignore(context, workspaceFolder, config.logDirectory).catch(
      (err) => {
    ...
    logSession.setSplitCallback((newUri, partNumber) => {
      ...
      this.outputChannel.appendLine(
        `File split: Part ${partNumber + 1} at ${newUri.fsPath}`,
      );
    });
    ...
    try {
      ...
      this.statusBar.show();
      this.outputChannel.appendLine(
        `Session started: ${logSession.fileUri.fsPath}`,
      );
    } catch (err) {
  ...
  async stopSession(session: vscode.DebugSession): Promise<void> {
    ...
    try {
      ...
      this.outputChannel.appendLine(
        `Session stopped: ${logSession.fileUri.fsPath}`,
      );
    } catch (err) {
    ...
    if (this.autoTagger?.hasTriggeredTags()) {
      ...
      this.metadataStore
        .setAutoTags(logSession.fileUri, autoTags)
        .catch((err) => {
          this.outputChannel.appendLine(`Failed to save auto-tags: ${err}`);
        });
      this.outputChannel.appendLine(
        `Auto-tags applied: ${autoTags.join(", ")}`,
      );
    }
