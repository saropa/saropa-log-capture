...
function deduplicateAssistantLines(lines) {
    ...
    for (const raw of lines) {
        ...
        if (m) {
            lastIndex.set(m[1], trimmed.length - 1);
        }
    }
...
function parseAssistantEntry(obj) {
...
function extractToolCall(block) {
    if (!isToolUseBlock(block)) {
        return null;
    }
    const b = block;
    const toolName = b.name;
    if (!relevantTools.has(toolName)) {
        return null;
    }
    const input = (b.input ?? {});
    return {
        ...
        isMutation: (0, ai_jsonl_types_1.isMutationTool)(toolName),
    };
}
/** Parse system-level warnings and errors. */
function parseSystemEntry(obj) {
...
//# sourceMappingURL=ai-jsonl-parser.js.map