...
exports.fetchGitHubActionsBuildInfo = fetchGitHubActionsBuildInfo;
exports.fetchAzureBuildInfo = fetchAzureBuildInfo;
exports.fetchGitLabBuildInfo = fetchGitLabBuildInfo;
...
function normalizeStatus(s) {
    ...
    if (lower === 'failure' || lower === 'failed' || lower === 'failed_' || lower === 'error') {
        return 'failure';
    }
    if (lower === 'cancelled' || lower === 'canceled' || lower === 'skipped') {
        return 'cancelled';
    }
    return 'success';
}
/** GitHub Actions: GET /repos/{owner}/{repo}/actions/runs?branch={branch}&per_page=1 */
async function fetchGitHubActionsBuildInfo(context, token) {
    if (!token) {
        return undefined;
    }
    ...
    const slug = (0, link_helpers_1.parseGitHubRemote)(remote);
    if (!slug) {
        return undefined;
    }
...
async function fetchAzureBuildInfo(context, pat) {
    if (!pat) {
        return undefined;
    }
    const { azureOrg, azureProject } = context.config.integrationsBuildCi;
    if (!azureOrg?.trim() || !azureProject?.trim()) {
        return undefined;
    }
    const branch = context.sessionContext.devEnvironment?.gitBranch?.trim();
    if (!branch) {
...
async function fetchGitLabBuildInfo(context, token) {
    if (!token) {
        return undefined;
    }
    const branch = context.sessionContext.devEnvironment?.gitBranch?.trim();
    ...
    const arr = await res.json().catch(() => undefined);
    const pipeline = Array.isArray(arr) ? arr[0] : undefined;
    if (!pipeline) {
        return undefined;
    }
    const statusStr = (pipeline.status ?? '');
...
//# sourceMappingURL=build-ci-api.js.map