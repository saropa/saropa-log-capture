...
exports.parseLogcatTag = parseLogcatTag;
 ...
 * Matches both standard LogInterceptor format (`Drift: Sent SELECT ...`)
 * and DriftDebugInterceptor format (`Drift SELECT: SELECT ...`).
 */
...
function extractSubTag(messageBody) {
    const bm = inlineTagPattern.exec(messageBody);
    if (bm?.[1]) {
        return bm[1].toLowerCase();
    }
    const cm = capsPrefix.exec(messageBody);
    if (cm?.[1] && cm[1].length >= 3) {
        return cm[1].toLowerCase();
    }
    return null;
}
/**
...
//# sourceMappingURL=source-tag-parser.js.map