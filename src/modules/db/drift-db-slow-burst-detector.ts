...
import type { ViewerSlowBurstThresholds } from "./drift-db-slow-burst-thresholds";

export const SLOW_QUERY_BURST_DETECTOR_ID = "db.slow-query-burst";

/** Per-session sliding window + cooldown (mutable; owned by host/tests/embed). */
export interface SlowBurstSessionState {
  hits: { readonly ts: number; readonly seq: number }[];
  lastEmitTs: number;
}

export type SlowBurstStateMap = Map<string, SlowBurstSessionState>;

export function createSlowBurstStateMap(): SlowBurstStateMap {
  return new Map();
}
...
function getOrCreateSession(map: SlowBurstStateMap, sid: string): SlowBurstSessionState {
  let st = map.get(sid);
  if (!st) {
 ...
 */
export function feedSlowQueryBurstDetector(
  ctx: {
  ...
  map: SlowBurstStateMap,
  t: ViewerSlowBurstThresholds,
...
export function pruneSlowBurstStateAfterTrim(map: SlowBurstStateMap, oldestKeptTimestampMs: number): void {
...
export function resetSlowBurstDetectorState(map: SlowBurstStateMap): void {
  map.clear();
}
