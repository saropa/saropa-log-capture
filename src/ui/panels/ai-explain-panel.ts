...
function buildExplanationHtml(context: AIContext, result: ExplainResult, nonce: string): string {
    const explanation = escapeHtml(result.explanation).replace(/\n/g, '<br>');
    const errorLine = escapeHtml(context.errorLine);

    const parts: string[] = [
        '<div class="toolbar"><button type="button" id="copy-btn" class="copy-btn">' + escapeHtml(t('panel.aiExplainCopyBtn')) + '</button></div>',
        '<div class="meta">Model: ' + escapeHtml(result.model) + (result.cached ? escapeHtml(t('panel.aiExplainCached')) : '') + '</div>',
...
export function showAIExplanationPanel(context: AIContext, result: ExplainResult): void {
    const panel = vscode.window.createWebviewPanel(
        VIEW_TYPE,
        t('panel.aiExplainTitle'),
