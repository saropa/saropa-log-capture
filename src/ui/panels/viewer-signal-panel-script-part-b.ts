...
export function getSignalScriptPartB(maxRecurringTextLen: number): string {
    ...
    /** Short badge text per signal kind. */
    var kindBadgeText = {
        error: 'ERR', warning: 'WARN', perf: 'PERF', sql: 'SQL',
        network: 'NET', memory: 'MEM', 'slow-op': 'SLOW',
        anr: 'ANR', permission: 'PERM', classified: 'ERR'
    };
    function kindBadge(kind) {
        var text = kindBadgeText[kind];
        if (!text) return '';
        return '<span class="signal-kind-badge signal-kind-badge--' + esc(kind) + '">' + text + '</span>';
    }
    ...
    function renderSignalTrends() {
        ...
        if (listEl) listEl.innerHTML = allSignals.map(function(s) {
            ...
            return '<div class="signal-env-row signal-trend-row' + sevCls + dimCls
                ...
                + '" title="' + esc(s.label) + '">'
                + '<span>' + icon + recurBadge + trendBadge + ' ' + kindBadge(s.kind) + esc(text) + '</span>'
