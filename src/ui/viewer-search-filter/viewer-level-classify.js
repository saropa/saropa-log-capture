...
function getLevelClassifyScript() {
    return /* javascript */ `
var looseErrorPattern = /\\b(?:error|exception)(?!\\s+(?:handl|recover|logg|report|track|manag|prone|bound|callback|safe))\\b|\\b(?:fatal|panic|critical)\\b|_\\w*(?:Error|Exception)\\b|Null check operator/i;
var strictErrorPattern = /\\w*(?:error|exception)\\s*[:\\]!]|\\[(?:error|exception|fatal|panic|critical)\\]|\\b(?:fatal|panic|critical)\\b|_\\w*(?:Error|Exception)\\b|Null check operator/i;
...
var warnPattern = /\\b(warn(ing)?|caution|fail(?:ed|ure)?)\\b/i;
