...
const perfRe = /\b(?:perf(?:ormance)?|dropped\s+frame|fps|jank|stutter|skipped\s+\d+\s+frames?|choreographer|doing\s+too\s+much\s+work|gc\s+(?:pause|freed|concurrent)|anr|application\s+not\s+responding)\b/i;
...
function countSeverities(bodyText) {
    let errors = 0;
    let warnings = 0;
    let perfs = 0;
    let anrs = 0;
    ...
    for (const line of lines) {
        ...
        if (looseErrorRe.test(msg) || logcatErrorRe.test(msg)) {
            errors++;
        }
        else if (warnRe.test(msg) || logcatWarnRe.test(msg)) {
            warnings++;
        }
        else if (perfRe.test(msg)) {
            perfs++;
            if (anrRe.test(msg)) {
                anrs++;
            }
        }
        else if (isFrameworkLine(msg)) {
            frameworks++;
        }
        else {
            infos++;
...
function isFrameworkLine(msg) {
    const tagMatch = logcatTagRe.exec(msg);
    if (tagMatch) {
        return tagMatch[1] !== 'flutter';
    }
...
//# sourceMappingURL=session-severity-counts.js.map