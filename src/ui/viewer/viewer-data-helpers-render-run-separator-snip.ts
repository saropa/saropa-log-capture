...
export const VIEWER_RENDER_EMBED_RUN_SEPARATOR = /* javascript */ `
    if (item.type === 'run-separator') {
        ...
        var dots = '';
        if (rs.errors > 0) dots += '<span class="run-sep-dot run-sep-dot-error" title="' + vt('viewer.runSeparator.errors') + '">' + rs.errors + '</span>';
        if (rs.warnings > 0) dots += '<span class="run-sep-dot run-sep-dot-warning" title="' + vt('viewer.runSeparator.warnings') + '">' + rs.warnings + '</span>';
        if (rs.perfs > 0) dots += '<span class="run-sep-dot run-sep-dot-perf" title="' + vt('viewer.runSeparator.perf') + '">' + rs.perfs + '</span>';
        if (rs.infos > 0) dots += '<span class="run-sep-dot run-sep-dot-info" title="' + vt('viewer.runSeparator.info') + '">' + rs.infos + '</span>';
        ...
        return '<div class="run-separator"' + idxAttr + '><div class="run-separator-inner">' +
            '<span class="run-sep-title">' + vt('viewer.runSeparator.runLabel', runNum) + '</span>' +
