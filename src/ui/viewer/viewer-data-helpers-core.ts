...
function calcItemHeight(item) {
    ...
    if (!item.peekOverride) {
        if (item.filteredOut || item.excluded || item.levelFiltered || item.sourceFiltered || item.classFiltered || item.sqlPatternFiltered || item.searchFiltered || item.errorSuppressed || item.scopeFiltered || item.repeatHidden || item.compressDupHidden || item.stackDedupHidden || item.metadataFiltered) return 0;
