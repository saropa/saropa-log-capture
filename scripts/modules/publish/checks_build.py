...
from modules.publish.constants import C, MAX_FILE_LINES, PROJECT_ROOT
from modules.publish.display import fail, fix, ok, warn
...
def check_l10n_bundles() -> bool:
    ...
    if audit.missing_from_bundle or audit.orphan_in_bundle:
        ...
        if added > 0 or removed > 0:
            ok(
                f"l10n English bundle synced: "
                f"+{added} added, -{removed} orphan(s) removed, "
                f"{kept + added} total"
            )
    else:
