...
### Session Management
- **Logs panel:** Slide-out panel listing past logs with filename, debug adapter, file size, date, and timestamp availability. Ctrl/Cmd-click to select multiple sessions; context menu applies to the selection (copy links/paths, export, tag, open, replay, reveal in file explorer). Hover any row to reveal a folder-opened icon that opens the log's containing folder in the OS file explorer (Explorer on Windows, Finder on macOS, file manager on Linux). Active sessions highlighted with a recording icon. **Orange dot** = log has new lines since you last viewed it; **red dot** = log updated in the last minute. Date filter dropdown: All time, Last 7 days, Last 30 days (persisted with display options).
...
**Accessibility:** The webview viewer is built for keyboard and assistive tech use. The main content has a `main` landmark; the icon bar is a `toolbar`; the log area has `role="log"` and a live region that announces line-count updates when filtering or loading. Slide-out panels (Options, Logs, etc.) have `region` landmarks and labeled controls; when you open a panel, focus moves into it, and **Escape** or the panel’s Close button returns focus to the icon bar. Native controls (buttons, selects, range inputs) are focusable and operable with Enter/Space. Replay controls, session/split navigation, and level filters are labeled. Collapse **divider** rows (between filtered or peek-hidden ranges) hide the redundant `─── N hidden lines · show ───` caption by default—the row and gutter chevron remain clickable; set `saropaLogCapture.accessibility.showCollapseDividerLabels` to true if you want that text visible. VS Code’s webview hosts this UI, so focus behavior at the editor boundary follows the host. For a full audit and remaining work, see [plans/028_plan-webview-accessibility.md](plans/028_plan-webview-accessibility.md) and [plans/028_webview-a11y-audit.md](plans/028_webview-a11y-audit.md).
