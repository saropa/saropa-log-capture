...
- **Investigations vs Recurring vs Performance:** **Investigations** are named collections you create—pin multiple sessions and files into one “case,” then search across them and export as a bundle (e.g. for a bug that spans several runs). **Recurring** (icon bar) is a read-only view of aggregated error patterns across sessions. **Performance** (icon bar) is a read-only view of performance data (traces, jank, etc.) for the current session or trends. So: Investigation = your container for a bug/case; Recurring = analysis of error patterns; Performance = analysis of perf data.
