...
- **Activity Timeline** — a chart of log volume over the session, so bursts and quiet stretches are obvious at a glance, with a small screenshot under any stretch that was captured.
- **Screen Visit Log** — which screens were entered, in what order, and how long each was on top, with visit badges, edge-dwell timing, and each screen's screenshot in its own row.
- **Interactive diagram** — zoom (wheel or the +/− buttons) and drag the background to pan; drag any card to rearrange the chart, and every arrow follows it. **Arrange by time** (⏱) lays the cards out along a wall-clock axis instead, so distance becomes elapsed time. The chart centers and fits on open and grows scrollbars instead of cropping when you zoom in; **Reset view** (⧉) restores both the zoom and the automatic layout. Double-click any node for a full detail card (type, time on screen, visit count, first/last times, source `file:line`, log line, actions, and issues), use **Center the fault** to jump to the crash node, **Export diagram as SVG** (⇩) to save it exactly as arranged for a bug report or a PR, or pop the chart out (⤢) into its own panel.
