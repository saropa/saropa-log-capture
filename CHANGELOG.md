...
- **Captured log files now record every raw line (capture-side deduplication bypassed).** Previously, `LogSession.appendLine` ran incoming lines through `Deduplicator.process()`, which folded identical-within-500ms runs into a single line with an `(xN)` suffix on disk. That collapsed per-line timestamps and broke the 1:1 mapping between file line numbers and what the app actually emitted. The write path now writes every formatted line directly; duplicate folding has moved entirely to the viewer layer where the unified outlined-dot collapse (`bugs/unified-line-collapsing.md`) handles it with click-to-expand and preserves per-line timestamps in the captured file. The `Deduplicator` class is retained (no longer invoked) in case a future opt-in capture-side mode reintroduces folding behind a setting. Effect: captured files grow proportionally to the duplicate volume in noisy logs, but line numbers, timestamps, and the ability to scroll through every individual occurrence are preserved.
- **Dedup folds are now click-to-expand.** Commit 4 of the unified line-collapsing rethink left dedup-fold survivors marked with the outlined severity dot but without a click handler — users could only unfold by flipping the global compression toggle. The render now stamps `data-dedup-count` on the survivor, `applyCompressDedupModes` records the list of hidden allLines indices as `compressDupHiddenIndices`, and a new `peekDedupFold()` in `viewer-peek-chevron.ts` reveals exactly that fold via the existing `peekOverride` / `peekAnchorKey` mechanism when the row is clicked. Clicking the expanded survivor again re-folds it. Scattered non-consecutive duplicates (possibly far from the survivor) are all revealed in one click without touching other folds.
- **Preview-mode stack trim now puts the outlined severity dot on the last visible frame (rule 3), not the header.** Commit 3 marked the header as a fallback; the plan said the frame at the preview cutoff owns the hidden-frames signal. Stack-frame rendering (extracted alongside stack-header rendering into `viewer-data-helpers-render-stack.ts`) detects when its parent header is in Preview mode and this frame is the highest-index visible app frame, then stamps `.bar-hidden-rows` plus a tooltip naming the hidden count (`5 more stack frames below (Preview mode · 3 of 8 shown)`). Headers in Preview mode no longer carry the state class — the indicator lives where the hidden content starts.
- **Removed dead `.hidden-chevron` / `.peek-collapse` CSS and the now-unreachable class skips in the viewport post-pass.** Nothing in the render pipeline emits those indicator elements after commit 2, so the 90-odd lines of CSS in `viewer-styles-decoration-bars.ts` and the `classList.contains('hidden-chevron')` / `…('peek-collapse')` skips in `findNextDotSibling` and the bar-bridge loop were dead code. Deleted together with the CSS-shape tests that pinned them; replaced with pin tests that assert the classes are gone.
- **Cross-type deduplication: stack-frame rows now participate in the non-consecutive `Compress lines` toggle and fold-survivors carry the outlined severity dot instead of an inline `(×N)` text badge.** Part 4 of the unified line-collapsing rethink (`bugs/unified-line-collapsing.md`). `lineDedupeKey` and `isLineEligibleForDupCompress` in `viewer-data.ts` now accept both `type === 'line'` and `type === 'stack-frame'` — a Drift SELECT flood that logs thousands of identical `DriftDebugInterceptor._log (...dart:92:5)` stack rows now folds to one surviving row per unique text when non-consecutive compression is on. The surviving row drops the prior `(×N)` `.compress-dup-badge` in favour of `.bar-hidden-rows` (outlined dot) plus a count tooltip on the row's outer div, matching the plan's "only decorations, no text badges" rule. Works identically for regular log lines and stack frames.
- **Retired the stack-header `▶ / ▼ / ▷` state triangle and the `[+N frames]` / `[+N more]` suffix; collapsed or Preview-trimmed stack headers now surface via the outlined severity dot.** Part 3 of the unified line-collapsing rethink (`bugs/unified-line-collapsing.md`). Stack-header rendering was extracted to `viewer-data-helpers-render-stack-header.ts` to stay under the 300-line file limit; the header's collapse state is read as before but the output drops the inline chevron character and suffix text. The outer div picks up `.bar-hidden-rows` + a stateful tooltip (`Stack trace collapsed · N frames · click to expand` or `Stack trace in Preview mode · N frames hidden · click to show all`) so the dot and hover text carry the signal. Combined with the `stackDefaultState` default flipped to Collapsed below, new noisy logs open with traces tucked away and a clear per-header indicator the user can click.
- **Stack-header text is selectable again.** Removed the `user-select: none` rule from `.stack-header` in `viewer-styles-content.ts`. The original 2021 collapsible-stack commit added it with no documented rationale; in practice it blocked users from copying function names, file paths, or error messages out of the header. Browsers distinguish plain click (fires the stack-group toggle) from click-drag (starts text selection) natively, so both affordances coexist without the CSS override.
- **`stackDefaultState` out-of-the-box default is now Collapsed** (was Expanded). Noisy logs — a Drift SELECT flood, logcat debug spam — otherwise render thousands of visible stack frames on first paint. Users who prefer the full trace on arrival switch to Expanded or Preview in Decorations → Stack frames. Default changed in both `viewer-deco-settings.ts` (initial value) and `viewer-decorations-state.ts` (reset-to-default handler) so the two stay in sync.
- **Retired the `▼` hidden-chevron and `−` peek-collapse glyphs; filter-hidden runs now surface via the outlined severity dot on the row that follows the gap.** Part 2 of the unified line-collapsing rethink (`bugs/unified-line-collapsing.md`). The render loop in `viewer-data-viewport.ts` no longer emits separate `<div class="hidden-chevron">` or `<div class="peek-collapse">` elements between visible rows; instead it injects `.bar-hidden-rows` onto the outer wrapper of the visible row that comes AFTER the hidden run (or the first row of a revealed peek group), along with `data-hidden-from` / `data-hidden-to` (or `data-peek-key`) and a title attribute carrying the tooltip. The severity dot on that row goes outlined via the CSS vocabulary added in the previous entry. `viewer-peek-chevron.ts` now delegates its click listener on `.bar-hidden-rows` and routes by which data-* attribute is present, replacing the old `.hidden-chevron, .peek-collapse` selectors. The old CSS and helper functions are left in place for now — a later cleanup commit removes the dead code once every hiding cause is wired to the new state.
- **Outlined severity-dot state for rows that have hidden rows associated with them (CSS vocabulary only, no wiring yet).** Part 1 of the unified line-collapsing rethink in `bugs/unified-line-collapsing.md`: a new `.bar-hidden-rows` modifier restyles the existing severity dot to a larger (0.9em vs 0.54em) outlined ring with an opaque center that matches the viewport background, so the ring still carries the severity color while the centre covers the connector bar behind it. No JS or rendering path currently applies the class — later commits wire it to the dedup-fold survivor, filter-hidden precedent row, user-collapsed stack header, and Preview-mode last-visible frame as each glyph (`▼`, `▾`, `× N`, `[+N frames]`, `[+N more]`) is retired in turn. Border thickness is `0.15em` so the ring scales with the log-zoom font size.
...
- **Lines streaming in after a level toggle now honour the filter on arrival.** With Database (or any other level) toggled off, new log rows arriving after the toggle stayed visible — `Drift SELECT:` lines, their inherited-level stack headers and stack frames, and every `N × SQL repeated:` notification row appended while the filter was off. `applyLevelFilter()` only runs on user actions (toggle click, preset change, persisted-state restore); it does not run per appended item. `addToData` and the repeat-collapse branch never consulted `enabledLevels`, so every streaming item was born with `levelFiltered` unset and rendered at full height until the next filter pass. A new `calcLevelFiltered(lvl)` helper now stamps `levelFiltered` at birth on every item type (log line, structured-file doc line, stack header, stack frame, repeat notification) and factors it into the initial height, so the first render already honours the current filter.
- **DB timestamp-burst and slow-burst markers now hide on arrival when their anchor line is already filtered.** The orphan check that hides a db-signal marker whose jump target is invisible only ran inside `applyDbSignalMarkerVisibility()` — which is invoked from `recalcHeights()`, itself triggered only by user interactions and a few specific code paths. New markers pushed by the DB detectors during streaming were born with `height = MARKER_HEIGHT` and stayed visible until the next `recalcHeights()` pass, so a `DB timestamp burst (3 queries at same instant)` marker could render with a green ribbon even though its anchor SELECT was already hidden by the Database level toggle. `applyDbMarkerResults` now runs the same anchor-visibility check at birth (using the shared `isNonMarkerItemEffectivelyHidden` helper, bounded to a short reverse scan since the anchor is the line pushed immediately before the marker in the same streaming batch) and stamps `markerHidden` + a zero initial height when orphaned.
- **ASCII art banners now render as one tight, perfectly-aligned block.** The detected box-drawing banner (e.g. the `╭─…─╮ DRIFT DEBUG SERVER ╰─…─╯` frame) had two visible issues: the 1.5x line-height used for prose logs left ~0.5em of empty space between every row so vertical strokes `│` on adjacent lines never connected, and the top-row's `╭` corner drifted rightward whenever the decoration prefix (line number, timestamp, PID/TID, tag) was wider or narrower than the hardcoded 13em hanging-indent assumption. Art-block rows now use `line-height: 1` with a matching compact `height`, and the virtual scroller's `calcItemHeight` returns the matching pixel height so prefix sums stay accurate and no drift accumulates below the block. The start-line decoration is pinned to a fixed `--deco-content-indent-em` slot via `display: inline-block; width` so the box-drawing art always begins at exactly the same column as the `│` bars on continuation rows regardless of decoration text length. Breathing room above and below the block moved from `margin` to `padding` so it is included in both the CSS height and the scroller's height, keeping subsequent rows exactly where the viewport expects them.
...
- **"Show DB signal markers" toggle** in the Filters panel under the SQL Commands tab. Turn it off to suppress every `DB timestamp burst (…)` / slow-query-burst marker in the log regardless of any other filter. Previously the only way to silence these markers was via the VS Code setting `saropaLogCapture.viewerDbDetectorTimestampBurstEnabled` (or the slow-burst toggle) and a reload — they ignored every in-viewer filter including the DB level circle, which is exactly what the user expected to control them.
- **Click a hidden-lines chevron to peek just that gap.** The `⋮` indicator between visible lines is now clickable. Clicking reveals exactly the hidden lines that chevron represents (level-filtered, excluded, search-filtered, user-hidden, auto-hidden, tier-filtered — every reason that counted into the gap) and drops a blue `−` un-peek marker above the first revealed line. Clicking the `−` collapses just that one group back; other peeked gaps are untouched. The global filter state is never modified, so a peek leaves your filters exactly as you set them. Shift+click still starts row selection (no collision with the peek action).

### Changed

- **DB signal markers now honour the log-line filters.** Timestamp-burst and slow-query-burst markers are "jump" links to a specific DB line in the log; when that anchor line is hidden by level, tier, source-tag, exclusion, scope, repeat-collapse, auto-hide or search filters, the marker would previously stay visible and clicking it silently did nothing (scroll skipped the height-0 target). Markers now disappear alongside their anchor so the marker you see is always a marker you can use.
- **Adjacent identical DB signal markers collapse into one with a `× N` count badge.** Previously, when the underlying DB lines were filtered out (e.g. DB level off) a cluster of timestamp bursts would render as a stack of ~10 green ribbons with nothing between them — technically correct but visually unreadable. Runs of consecutive db-signal markers with no visible log lines between them now merge into a single marker carrying the count; adding any visible line between breaks the run.

### Fixed

- **`[console]` now registers as a source tag on timestamp-prefixed log lines.** Console/SDA output arrives as `[16:07:58.532] [console] [log] …` — the leading bracket is a timestamp, not a tag, and the noisy-tag guard correctly rejected it. But the parser then returned null without looking further, so the meaningful secondary tag (`[console]`, `[log]`, …) never reached the filter chips. Logcat tags like `WindowManager` / `ActivityManager` worked because their prefix is a non-bracket logcat format. The parser now falls through to the inline-tag scan when the leading bracket is noisy, so `console` (and other letter-led secondary brackets) show up in the Message Tags section alongside logcat tags. Drift SQL lines in this format still map to `database` as before.
- **Flutter `════ Exception caught by <library> ════` banners are now classified as errors and grouped as one block.** Flutter renders a multi-line banner around every framework exception (rendering / widgets / scheduler / gesture / services / image resource). The opening title line was previously classified as `info` because `Exception caught by` has no colon, bracket, or underscore that the strict/loose error patterns look for — so the entire incident (40+ lines of overflowing `RenderFlex` dumps, widget paths, text-span debug output) dropped out under the Errors/Warnings filter. Two changes: (1) `Exception caught by` is now a structural error pattern in both the extension-side and webview classifiers, so the opening line classifies as error in strict and loose modes regardless of user keyword config; (2) the viewer now detects the opening `════ Exception caught by …` rule and the closing `════════…` rule and tags every line between them with a `bannerGroupId`, forcing level to `error` (so the whole block stays visible under filters) and rendering a connected left accent rail + background tint so the incident reads as one cohesive band instead of 40 scattered info lines.
...
- **Hidden-lines indicator is now visible.** The chevron that appears between visible log lines when non-blank lines have been hidden (by filters, exclusions, or manual hide) was a tiny `▸` at 0.75em crammed just to the left of the severity-dot column — effectively invisible against the dot it abutted. Replaced with a vertical ellipsis (`⋮`) centered in the severity-dot column at 2em, so the glyph reads as a row-sized gap in the dot timeline rather than a decorative hint. The tooltip (count + reasons) is unchanged.
- **Hidden-lines indicator no longer lands in the clipboard.** Drag-selecting across a gap and pressing Ctrl/Cmd+C was pulling the chevron glyph into the copied text (you'd paste and find stray `▸` characters between your log lines). The glyph is now drawn via CSS `::before { content }` instead of a DOM text node, so it's invisible to every copy path (native selection, Select All, programmatic clipboard) and can never appear in copies.
- **DB signal markers now actually disappear when hidden or collapsed.** The marker visibility + consecutive-collapse passes set `markerHidden` / `markerCollapsed` on each marker, but `calcItemHeight` still returned `MARKER_HEIGHT` for every marker regardless of those flags — so orphaned and collapsed markers kept occupying a row even though the filter pass had run. `calcItemHeight` now honours both flags and returns 0 for hidden or collapsed markers, making the filter pass's output match what the user sees.
- **Main log file is no longer loaded a second time as its own "external sidecar".** `findSidecarUris()` walks the report folder for files starting with `<baseName>.` and ending with `.log` — a filter the primary itself satisfies (e.g. `20260421_160343_contacts.log` starts with `20260421_160343_contacts.`). The viewer then appended the primary a second time through the external-sidecar path, which escapes HTML but does not strip the `[HH:MM:SS.mmm] [category]` prefix or convert ANSI. Users saw phantom rows with `[16:13:49.489] [console] [log] [32m…` visible in the body and entry-time pinned via `extractTimestamp` — duplicates of every real log line. The discovery loop now excludes `<baseName>.log` so only true sidecars (`<baseName>.logcat.log`, `<baseName>.drift-advisor.log`, etc.) enter the external-append path. Affects every caller of `findSidecarUris` (viewer load, timeline, `.slc` export/bundle, context popover, external-logs command).
...
- **Session Groups: auto-bundle a debug session's logs with its sidecar logs into one entry.** When a DAP debug session starts, the extension mints a `groupId`, stamps every log file produced in a configurable window around the session (before/during/after) with that id, and renders them in the Logs panel as one collapsible block. The primary row (the main debug session log) carries a chevron and a `+N` count badge; secondaries indent under a muted tether line; hovering any member dims the whole group together. When collapsed, the primary's severity badges show the summed totals across every member; when expanded, each row shows its own per-file counts. Three new settings control the behavior: `saropaLogCapture.sessionGroups.enabled` (on by default), `.beforeSeconds` (default 10 — files created this many seconds before the DAP start are claimed), and `.afterSeconds` (default 10 — the group stays open this long after DAP end to catch late-flushed sidecars). The collapsed state persists per groupId across webview reloads.
- **Session Groups: manual Group / Ungroup via the right-click menu.** When auto-grouping gets something wrong — or the user just wants a different split — ctrl-click to multi-select rows, right-click → **Group Selected Sessions** to bind them under a fresh groupId. User intent overrides the "never re-claim" rule: any existing groupId on the selected rows is cleared first, then the new id is stamped. Right-click any group member → **Ungroup Session** to dismantle the group; every other file sharing the same groupId gets ungrouped in one batch. On a single row the Group label reads "Start New Group" and clears that row's auto-assigned groupId — a useful detach-from-auto-grouping action. Both commands are also available from the Command Palette.
- **Session Groups: Open as Merged Group.** Right-click a group member → **Open as Merged Group** to load every file in the group into the viewer as one merged stream. Files sharing the primary's basename auto-merge through the existing sidecar pipeline; cross-basename members (typical of manually grouped files) are appended with a filename-derived source tag like `external:logcat` or `external:drift-advisor`, and the existing source-tag filter lets you toggle each one on/off inside the merged view. Viewer title switches to "Session Group (N files)" after the load.
- **Session Groups: Add Group to Collection.** A new right-click action pins a group to a collection as one logical source. At search and export time the group expands to every file currently sharing its groupId, so if you manually regroup or ungroup later the collection reflects today's group composition automatically. Group pins count as one against the `MAX_SOURCES_PER_COLLECTION` cap.
- **Session Groups: retention is group-aware.** The file-retention sweep that enforces `maxLogFiles` now respects groups: it skips any file whose groupId matches a currently-active DAP session (never touches a group that's still being written to) and, for closed groups, trashes every member atomically so half a group can't end up orphaned. Ungrouped files follow the existing oldest-first rule.

### Changed

- **Scroll map markers now leave a 1px gap so adjacent lines read as discrete rows.** Severity ticks, search-match bars, and the neutral presence band previously painted at a fixed 3px/2px height; when the minimap had enough vertical room that consecutive log lines were ≥3px apart on the strip, the bars touched and formed a solid wall with no visible row separation. Each bar's height is now capped to `pitch − 1` (the per-line vertical allotment on the minimap minus one), so a gap is reserved whenever space allows. On dense logs where the per-line pitch is sub-pixel, bars stay at 1px with no gap — behavior there is unchanged.
- **Logs panel right-click menu: Export and Copy grouped under flyout submenus.** The six Export targets (HTML, CSV, JSON, JSONL, .slc Bundle, Loki) now sit under a single "Export ▸" entry, and the two Copy targets (Deep Link, File Path) sit under a single "Copy ▸" entry. Hover the entry to open the flyout. No behavior changed — actions still accept multi-select and still dispatch the same `sessionAction` messages. Flyouts flip to open leftward automatically when the menu is near the right edge of the viewport.

### Fixed

- **Critical error badge overlapped the log text and duplicated the severity dot.** The inline `🔥 CRITICAL` pill (red border, bold text) was prepended to every line classified as critical, crowding the log content and appearing alongside the red severity dot already painted in the gutter. The badge is now a bare fire emoji positioned over the severity dot (same gutter slot as the `[class*="level-bar-"]::before` indicator), with no "CRITICAL" text and no red border. The emoji is still interactive — hovering opens the same cross-session error panel and clicking still routes to the full Analyze view — because `.error-badge-interactive` is retained on the new `.critical-fire-icon` span and the hover listener now hooks on that class rather than `.error-badge`. The duplicate gutter dot is hidden via a `:has(.critical-fire-icon)` rule so there's exactly one visual severity indicator per critical line.
- **"No analyzable tokens found in this line." when clicking Analyze on lines without classic error/HTTP/URL/quoted signatures.** `extractAnalysisTokens` bailed on Android framework lines like `ActivityManager » Slow operation: 263ms so far, now at finishAttachApplicationInner: after updateOomAdjLocked` because none of the existing patterns (error-class suffix, HTTP 4xx/5xx, `/url/path`, quoted string, `Class.method` dot-form) matched — even though the line has several useful identifiers to search on. A fallback pass now extracts PascalCase class names (`ActivityManager`) and camelCase method names (`finishAttachApplicationInner`, `updateOomAdjLocked`) — but only when no other tokens matched, so error-rich lines don't get flooded with noisy identifier hits. Both patterns require ≥2 word segments so single capitalized words ("Slow") don't qualify.
- **File Scope filter radios greyed out once the log viewer has focus.** In Filters → File Scope, everything except **All logs** (Only workspace / Only package / Only directory / Only file) was permanently disabled whenever the log viewer webview had focus — which is exactly when the user opens the Filters panel to use them. The scope-context listener in `activation-listeners.ts` was rebuilding the context from `vscode.window.activeTextEditor` on every `onDidChangeActiveTextEditor` event, and VS Code fires that event with `undefined` whenever focus moves to a non-text surface (webviews, sidebar, settings UI). That null context wiped the workspace/package/directory/file paths the webview uses to decide which radios to enable. Fixed by having the listener ignore `undefined` editor firings, preserving the last known scope context while focus is on the log viewer panel. The cold-start seed still passes `undefined` through so a first-launch with no file open keeps showing the "Open a source file to enable scope filters" hint. Also added concise per-radio explanations: when a radio is dimmed, hovering its label now reads the specific reason (e.g. "This file is outside any workspace folder", "No package manifest (pubspec.yaml, package.json, etc.) was found above this file"), and the status line above the radios surfaces the outermost missing layer ("…— related scopes are unavailable") so a partial-dim state reads as expected behavior rather than a bug.
- **"Show native scrollbar" could not be turned off once enabled.** Toggling the setting via the Scroll map & scrollbar submenu (or the right-click compact menu on the minimap / scrollbar) correctly flipped the checkbox and moved the Top/Bottom jump buttons back inward, but the 10px Chromium scrollbar itself stayed on screen. Chromium paints `::-webkit-scrollbar` once per scroll container and caches the composited layer; the previous fix cycled `overflow-y: auto → hidden → auto` to force a repaint, which worked for 0 → 10px (the layer had to be created fresh) but failed for 10px → 0 (the cached layer survived the overflow cycle). `applyScrollbarVisible` in [viewer-script.ts](src/ui/viewer/viewer-script.ts) now briefly sets `display: none` on `#log-content`, forces a synchronous reflow via `offsetHeight`, and restores the previous display value — tearing down the render tree so Chromium rebuilds the scroll container and re-reads `::-webkit-scrollbar` from scratch. `scrollTop` is captured before the toggle and restored afterward so the viewer does not jump to the top.
- **Log Sources radios (All / Warn+ / None) had no effect on the log view.** In Filters → Log Sources, toggling Flutter DAP / Device / External between All, Warn+, and None changed the radio state but didn't hide or reveal any lines — the bulk of a typical Flutter app's output is plain `print()` / `debugPrint()` text arriving via the Debug Adapter Protocol (`source='debug'`, category stdout/stderr/console). Those lines aren't stack frames, aren't logcat-prefixed, and aren't launch boilerplate, so `classifyFrame()` returned `undefined`; the tier fallback in `viewer-data-add.ts` then left `tier=undefined` on the line, and `isTierHidden()` has an early `if (!item.tier) return false` bail-out that lets untagged lines through every filter mode unconditionally. The Flutter DAP radio therefore couldn't control the very output it claimed to control. Fixed by defaulting unclassified debug-source lines to `tier='flutter'` in both `viewer-data-add.ts` (main ingest path) and `viewer-data-add-repeat-collapse.ts` (repeat-collapse branch) — `classifyLogLine()` already tags VM Service connect lines, "Launching…" build output, and logcat explicitly as `device-other`, so anything still reaching the fallback is legitimate Flutter app text. External sources (terminal, browser, drift-perf, saved log files, ai-bash/ai-prompt/ai-edit) continue to be tagged `external` by the non-debug branch.
- **Ungroup Session and Open as Merged Group silently ignored sibling files.** The fan-out logic in `commands-session-groups.ts` rebuilt sibling URIs with `vscode.Uri.joinPath(logDir, relKey)`, where `relKey` came from `asRelativePath(uri)`. That produces either a workspace-relative path (common case — joining re-prefixed the workspace folder, giving `/ws/reports/reports/a.log`) or an absolute path when no workspace folder matches (test harness — joining concatenated, giving `/tmp/logs/tmp/logs/a.log`). Either way the rebuilt URI no longer round-tripped through `asRelativePath` to the same key, so the subsequent `stampGroupIdBatch` call looked up a nonexistent entry and the ungroup/merge operation silently no-op'd for every sibling beyond the one the user clicked. Fixed by introducing `keyToLogUri(reference, key)`, which inverts by matching how the key was produced: absolute keys rebuild with `Uri.file`, workspace-relative keys rejoin with the reference URI's workspace folder. Both `runUngroupSession` (the fan-out clear loop) and `pushGroupMembers` (the merge-open expansion) now use the helper so the rebuilt URIs match the stored keys exactly.
