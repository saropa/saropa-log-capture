...
- **Default tier filter for both Flutter and Device changed to Warn+.** Previously Flutter defaulted to "All" and Device to "None". Both now default to "Warn+" so only warnings and errors are shown initially, reducing noise while still surfacing important issues from both tiers.
...
- **Lint diagnostic enrichment on signals.** Error and warning signals whose example text references a source file (e.g. `lib/main.dart:42`) are enriched with active VS Code diagnostics at that location. Diagnostics from any provider (saropa_lints, Dart analyzer, ESLint, TypeScript, etc.) appear as ` | Lint: code (severity, source)` appended to the signal detail. This correlates runtime signals with static analysis findings.
