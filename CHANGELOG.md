...
- **Aggregate Crashlytics stats:** Crash detail view now progressively loads aggregate device/OS distribution from the Crashlytics `issueStats` API endpoint, covering all events (not just the cached 5). Renders using existing distribution bar chart with deterministic placeholder positioning.
