...
- **App Quality Insights dashboard** — a new full editor-tab view (command: **Saropa Log Capture: Open App Quality Insights**) modeled on Android Studio's screen: a toolbar (time range, refresh, Open Firebase Console) over a 3-pane layout — issues list, selected-issue stack trace, and a device/version breakdown. It reads from the same Play Developer Reporting data layer and reuses the sidebar's crash-detail and distribution renderers (so stacks are app/framework-classified and source frames are click-to-open). Built as standalone files ([app-quality-insights-panel.ts](src/ui/panels/app-quality-insights-panel.ts), [-render.ts](src/ui/panels/app-quality-insights-render.ts), [-styles.ts](src/ui/panels/app-quality-insights-styles.ts)). First UI slice of [plan 054](plans/054_plan-app-quality-insights.md). Includes source/type tabs (All / Crashes / ANRs / Non-fatals, with counts), a text search, and an app-version filter — all client-side over the loaded issues. The right pane shows a **true per-issue device + Android-version breakdown** from the Play error-count metric set (real aggregate counts, not a sample), via [play-reporting-metrics.ts](src/modules/crashlytics/play-reporting-metrics.ts) (`issueId`-filtered, `reportType` dimension required, end clamped to data freshness); it falls back to the sampled-event estimate if the metric set is unavailable. (Server-side device/OS *filters* on the issue list remain a follow-up in the plan.)
