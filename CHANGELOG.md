...
- **Session display options:** Four toggle buttons in the Project Logs panel header control how session filenames are displayed: **Dates** (strip leading/trailing datetime patterns like `YYYYMMDD_HHMMSS_`), **Tidy** (normalize to Title Case with spaces), **Days** (group sessions under day headings like "Tue, 3rd Mar 2026"), and **Old** (reverse sort, oldest first). All options persist per-workspace via `workspaceState`. The same transforms apply to both the webview session panel and the native tree view.
- **File modification date in session list:** Session items now show the file's last-modified date (e.g. "Jan 15" or "Jan 15, 2024") before the file size in the metadata line.
