...
- **Webview localization groundwork** — the log viewer was English-only; neither existing l10n pipeline (`package.nls*` for `package.json`, `vscode.l10n.t()` for extension-host UI) reached the webview. A new bridge ([viewer-l10n-inject.ts](src/ui/provider/viewer-l10n-inject.ts)) resolves webview string keys ([strings-webview.ts](src/l10n/strings-webview.ts)) host-side and ships them into the iframe as `__VT` with a client-side `vt(key, …args)` helper, so render-time strings (built where `vscode.l10n.t()` can't run) become localizable. Host-assembled HTML uses `t()` directly. The collapsible stack/tree header tooltips are the first consumer. English source strings only; translations flow through the existing `l10n/bundle.l10n.*.json` process and fall back to English until filled (no behavior change today). Tracked sweep in [plan 053](bugs/053_plan-webview-localization-sweep.md).
