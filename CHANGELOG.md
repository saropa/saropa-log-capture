...
- **Crash stacks are linked to your code (source line + git blame)** — in the dashboard detail, each app stack frame that resolves to a workspace file now shows the **actual source line** and **git blame** for it (`author · when · short-SHA`), streamed in just after the stack renders. New [crash-frame-context.ts](src/modules/crashlytics/crash-frame-context.ts) resolves the file (absolute / workspace-relative / basename search), reads the line, and runs `git blame -L` (capped at 8 frames, 5s each, all best-effort). This is the editor-native angle the Android Studio panel / Play Console can't offer — it has the working tree and git. Plan [054](plans/054_plan-app-quality-insights.md).
- **Crashlytics issues are cached to disk and stay visible offline** — every successful fetch persists the issue list to `.saropa/cache/crashlytics/issues.json`; when a later refresh can't reach the API (offline or error), the dashboard shows that cached list instead of going empty, labeled with a stale banner ("Showing cached issues — could not refresh") so it never masquerades as fresh. See [crashlytics-io.ts](src/modules/crashlytics/crashlytics-io.ts) (`readCachedIssues`/`writeCachedIssues`) and [crashlytics-api.ts](src/modules/crashlytics/crashlytics-api.ts).
- **App Quality Insights dashboard** — a new full editor-tab view (command: **Saropa Log Capture: Open App Quality Insights**) modeled on Android Studio's screen: a toolbar (time range, refresh, Open Firebase Console) over a 3-pane layout — issues list, selected-issue stack trace, and a device/version breakdown. It reads from the same Play Developer Reporting data layer and reuses the sidebar's crash-detail and distribution renderers (so stacks are app/framework-classified and source frames are click-to-open). Built as standalone files ([app-quality-insights-panel.ts](src/ui/panels/app-quality-insights-panel.ts), [-render.ts](src/ui/panels/app-quality-insights-render.ts), [-styles.ts](src/ui/panels/app-quality-insights-styles.ts)). First UI slice of [plan 054](plans/054_plan-app-quality-insights.md). Includes source/type tabs (All / Crashes / ANRs / Non-fatals, with counts) plus **text search and version / device / OS filters** — all client-side over the loaded issues. Device and OS are downloaded **lazily** — only when you first open those dropdowns — as per-issue maps (`fetchIssueFilterIndex` queries the error-count metric set grouped by `issueId` + `deviceBrand` / `apiLevel`) and then indexed onto each row in the webview, so the common open-and-scan path skips those extra queries entirely and later filtering is instant. The right pane shows a **true per-issue device + Android-version breakdown** from the same metric set (real aggregate counts, not a sample), via [play-reporting-metrics.ts](src/modules/crashlytics/play-reporting-metrics.ts) (`issueId`-filtered, `reportType` dimension required, end clamped to data freshness); it falls back to the sampled-event estimate if the metric set is unavailable.
...
- **Clicking a Crashlytics sidebar issue now opens the full App Quality Insights dashboard focused on that issue** — like clicking a log line opens the log viewer, instead of a cramped inline expander in the narrow sidebar. The sidebar is now a list of launchers (each row shows ↗ and opens the dashboard, deep-linked to the issue and auto-selected); the dead inline-detail path (`toggleDetail`, `fetchCrashDetail`/`crashDetailReady`, `handleCrashDetail`) was removed. See [viewer-crashlytics-panel.ts](src/ui/panels/viewer-crashlytics-panel.ts) and [app-quality-insights-render.ts](src/ui/panels/app-quality-insights-render.ts).
