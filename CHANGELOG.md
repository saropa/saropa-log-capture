...
- **Decoration prefix column is now a fixed, content-sized column** — with line numbers / timestamps on, the message text could drift a few ems line-to-line because the decoration prefix span was content-sized while the hanging-indent math assumed a fixed width; and that assumed width was a static worst-case that always reserved space for timestamp, PID/TID, and tag columns even when only the counter was shown, leaving a large empty gap between the severity bar and the text. The prefix column width is now computed from the decoration parts that are actually enabled — each disabled part contributes zero — and the prefix span is pinned to that width (`display: inline-block`), so the message column starts at the same x on every decorated line and the gap is only as wide as the columns in use. The existing `padding-left` / `text-indent` model is otherwise unchanged, so wrapped SQL and error lines still align under the message column.
