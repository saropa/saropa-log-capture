...
• **DB_15 — annotate-line host API & VM coverage** — **`applyDbAnnotateLineResultToLineItems`** / **`applyDbAnnotateLineResultsToLineItems`** in **`db-detector-framework.ts`** for batch/test line arrays; **`viewer-db-detector-annotate-line.test.ts`** runs the real embed chunks in **`node:vm`** (detector-driven annotate, unknown-seq no-op, height delta). Shared embed helper **`driftSqlSnippetFromPlain`** for dbInsight fallback snippet text.
