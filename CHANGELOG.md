...
- **Decoration prefix column is now structurally fixed-width** — with line numbers / timestamps on, the message text could drift a few ems line-to-line because the decoration prefix span was content-sized while the hanging-indent math assumed a fixed width. The prefix span is now pinned to that width (`display: inline-block` + explicit `width`), so the message column starts at the same x on every decorated line regardless of counter, timestamp, PID/TID, or tag content. The existing `padding-left` / `text-indent` model is otherwise unchanged, so wrapped SQL and error lines still align under the message column.
