...
- **All expand/collapse affordances on regular log rows collapse into a single clickable line-number column** — the in-tag-slot dedup chip + the between-row `▶ hidden` / `▼ hidden` / `▶ stack` divider rows still had visual problems: between-row labels overlapped the tag chips of the rows immediately above and below (zero-height divider + `translateY(-50%)` parks the label across the row boundary at the same x-column as adjacent rows' tags), and multiple dividers at the same row pair stacked on top of each other. Retired the divider concept entirely. Filter-hidden gaps, dedup-fold survivors, and expanded peeks now surface ONE way: a small `▶` (collapsed) or `▼` (expanded) chevron rendered immediately right of the line-number digits in the decoration prefix. The whole `.deco-counter-row` wrapper (line-number digits + chevron) is the click target — aim at the number OR the chevron, same action. Click routing is keyed off `data-affordance-kind` (dedup / gap / peek) so all three reuse the same delegate path in [viewer-peek-chevron.ts:53](src/ui/viewer/viewer-peek-chevron.ts#L53). A per-render pre-pass (`computeRowAffordances` in [viewer-data-divider.ts](src/ui/viewer/viewer-data-divider.ts)) stamps `_hiddenAfter` on rows that own a filter-hidden gap below them and `_triggeredPeekKey` on rows that opened a currently-expanded peek, so the chevron knows what to dispatch without each row scanning forward. Stack-header rows keep their inline `.stack-toggle` because they have no line-number prefix to host a counter-row.
