...
### Fixed
- **Severity connector stripe widened from 2 px to 4 px so per-row hue is legible** — the 70% opacity bump in 7.11.2 made the connector vivid enough that the 1-row empty gap at a color transition was no longer ambiguous, but at the original 0.14em (~2 px) stripe width the per-row color binding was visually irresolvable. Adjacent rows of different levels read as one fuzzy continuous line because the eye can't see hue clearly on a 2-pixel-wide vertical stripe — both the previous "joining between colors" and the post-bump "fixed color, no breaks" reports trace to that. Bumped to 0.28em (~4 px) and dropped opacity back to 45% so row backgrounds bleed through enough to preserve hue contrast at the higher width. Stripe is now re-centered under the dot at `left: 0.82em` (dot at 0.74em + 0.22em radius − half the new width).
