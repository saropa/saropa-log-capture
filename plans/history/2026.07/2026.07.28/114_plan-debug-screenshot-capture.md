...
The icon opens a **capture options menu** (revised 2026-07-28; supersedes the earlier bare on/off click): master Screenshots toggle, the three trigger checkboxes (errors on by default; warnings and navigation off), a Capture now action (the manual command), an Open gallery entry, and a muted info line showing the active cooldown and per-log cap. Checkbox changes write the corresponding `integrations.screenshots.*` setting; the host re-broadcasts settings so the menu, the Integrations panel checkbox, and Settings JSON stay in sync. The icon stays always visible and dims when the master toggle is off. The counter appears next to the icon once captures exist; it increments live as captures land (broadcast via the existing ViewerBroadcaster path).
