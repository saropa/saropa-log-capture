...
- **Exclusions** — Regex/pattern hide; applies to all sources or per-source (TBD).
- **App only** — Hide framework/system DAP output; only for debug source.
- **Level filter** — Error, warning, info, etc.; applies to all sources that have level classification.
...
- Viewer opens “session” = main log + list of sidecars; one content area (unified view) with **source filter** (checkboxes). When multiple sources are selected, merge in memory by timestamp (or by order) and show one scroll with source badges per line.
- Pros: No change to main log format; reuses existing sidecar pattern.
- Cons: “All sources” view requires loading multiple files and merging (by received-at or parsed time).
...
- **Control:** Checkboxes. If only main log is present → no source UI (everything is debug). If main + terminal and/or external sidecars exist → show checkboxes: “Debug”, “Terminal”, “app.log”, … (one per external), plus “All” (select all). User can select any combination of sources; unified view shows one merged list for the selected sources.
...
- **Unified file (Phase 4):** Post-step at session end: read main log + terminal sidecar + external sidecars, emit merged stream with source tags to `basename.unified.log` or JSONL. Viewer can open this file and still filter by source.
...
|-------|--------|--------------|
| **1. Source in viewer** | Add source as a filter dimension when multiple “files” are present (main + terminal sidecar; later + external). Unified view: one list, source filter (checkboxes). “Just debug” = only main log content. | Source filter UI (checkboxes); load sidecars when opening a log; merge view when multiple sources selected; presets can include `sources`. |
...
- **Manage overwhelm** = **Source filter** (checkboxes: Debug, Terminal, externals, “All”) plus existing **noise filters** (presets, exclusions, app-only, categories, levels, search). **“Just debug output”** preset and presets with optional **sources**.
...
- **Data model:** **Source id** per line (debug | terminal | external:label). Viewer derives source from which file the line came from; filtering by source via checkboxes.
