...
1. **H12 — IN PROGRESS (the audit finding is partly stale).** Route the hardcoded panels through `t()`/`vt()`. Current state after re-checking the code (the audit was 2 days old; the other workstream localized several panels since):
   ...
   - **F12 done; remaining = on-device F5 confirmation only.** Every hardcoded/concatenated user-facing string across the audited panels is now localized. English output is byte-identical (the wraps don't change text), so the only residual risk is a mis-wired key showing a raw `signals.x`/`viewer.x` string — confirm by opening each panel (F1 shortcuts; Signal Report incl. Copy/Save; analysis stack-trace; Performance + Error-rate tabs) and checking no raw keys appear.
