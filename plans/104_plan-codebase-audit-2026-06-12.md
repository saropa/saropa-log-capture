...
7. ~~**M6/M8/M13/M15/M16** correlation/source/AI/root-cause~~ **DONE 2026-06-13** — M6: correlation dedup compares the real anchor (`events[0]`) via `sameAnchor`, not any shared event. M8: `package:` URIs resolve under `lib/` (`<root>/lib/...`). M13: AI dedup reads structured `message.id`, not a raw-text regex. M15: `excerptKey` uses the leading 80 chars (distinguishing content), not trailing. M16: 4xx confidence regex uses `\b` not `\s`. Tests: correlation-detector (13), source-resolver (5, tightened to pin lib/), ai-jsonl-parser (23), build-hypotheses (29) passing. *(M17 SQL-aggregate subtraction still TODO.)*
