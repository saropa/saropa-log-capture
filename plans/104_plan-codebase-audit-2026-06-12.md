...
1. **H12 — OPEN (needs on-device verification).** Route the hardcoded panels through `t()`/`vt()`: keyboard-shortcuts, vitals, crashlytics-setup, signals report, analysis sub-modules, session-history tree. ~200+ strings across 6 panel areas. **Deliberately not bulk-localized in this audit pass:** each localized panel is rendered UI that must be re-verified across light/dark/high-contrast/RTL/dyslexia modes and small/large viewports — which requires running the extension (the audit harness can't render webviews). Bulk-localizing blind would ship unverified panels, violating the production-quality bar. Recommend doing it as dedicated, on-device-verified passes (one panel per pass), `verify:nls-coverage` + `vt()` wiring, brand tokens kept literal. The `.one`/`.many` + `t()`/`vt()` pattern is established (see L14 and `analysis-panel-render.ts`).
