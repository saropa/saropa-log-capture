...
1. **H12 — IN PROGRESS (the audit finding is partly stale).** Route the hardcoded panels through `t()`/`vt()`. Current state after re-checking the code (the audit was 2 days old; the other workstream localized several panels since):
   ...
   - ⏳ **Webview-script stragglers** — a few client-side `vt()`-less progress/label strings remain in **webview scripts** (e.g. `analysis-panel-script.ts:14` `'Analyzing... {done}/{total} complete'`, and L13 concatenations in `viewer-error-rate-tab`/`viewer-signal-panel-script-*`). These build text in the iframe, so they need the webview l10n map (`getWebviewL10nScript`/`__VT`) injected into those panels first — a small infrastructure step, NOT a blind wrap. Track separately + F5-verify.
   - **All localized panels need an F5 render check** — English output is byte-identical (the wraps don't change text), so the risk is a mis-wired key showing a raw `signals.x` string; verify across themes/RTL/viewport.
