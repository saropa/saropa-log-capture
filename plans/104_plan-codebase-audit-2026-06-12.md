...
1. **H12 — IN PROGRESS (the audit finding is partly stale).** Route the hardcoded panels through `t()`/`vt()`. Current state after re-checking the code (the audit was 2 days old; the other workstream localized several panels since):
   ...
   - ⏳ **signals report** (`signal-report-overview/render/details/history/ecosystem/related.ts`) — host-built HTML, NOT localized. Complication: each builds an HTML panel AND parallel **markdown export** functions (`signal-report-markdown.ts`) that share helpers (`gatherStats`, `outcomeLabel`); the markdown is a GitHub/support artifact that should stay English. Needs per-function HTML-vs-markdown separation (a `labelKey` field), not a blind wrap. ~7 files.
   - ⏳ **analysis sub-modules** (`analysis-frame-render.ts` and remainders) — partially localized; finish the stragglers + L13 concatenation there.
   - ✅ **session-history tree** — already localized (`session-history-grouping`/`-helpers` use `t()`; survey showed ~0 literals).
   - **Remaining (signals + analysis) needs on-device verification** — each is rendered webview UI; verify across light/dark/high-contrast/RTL/dyslexia + viewports via F5. Recommend dedicated per-file passes; the `t()`/`vt()` + `.one`/`.many` pattern is established (`strings-kbd.ts`, `analysis-panel-render.ts`).
