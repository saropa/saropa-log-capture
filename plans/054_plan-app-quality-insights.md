...
- **Crash-free users exact parity (T3.3) — DONE (2026-06-12).** Schema-verified (6d-style check): `crashRateMetricSet` exposes `userPerceivedCrashRate` (distinct-user) + `distinctUsers` alongside the session `crashRate`. The crash query now requests both `crashRate` and `userPerceivedCrashRate` in one call. Parsing extracted to pure `vitals-metrics.ts` (`latestMetric` / `metricSeries`), selecting BY NAME (not "first metric") and tolerant of both response shapes (array and name-keyed) — 4 `node:test` cases. `userCrashRate` added to `VitalsSnapshot`; the panel shows "Crash-free users" (100 − userPerceivedCrashRate) beneath "Crash-free sessions".
- **Device states foreground/background panel (T3.2) — DONE (2026-06-12).** The `appProcessState` dimension confirmed in the schema read (6d) is now surfaced. `fetchProcessStates` queries it off the error-count metric set (reusing `queryDimension`/`parseMetricRows`); `getProcessStates` wrapper. `renderProcessStates` (in `analysis-crash-detail.ts`, reusing `renderDistributionBar`) renders a true-aggregate "Device states" `cd-tile`; `streamDeviceStates` posts `crashlyticsDeviceStates` (streamed into the detail like project-insights / log-correlation — and like those siblings it is not in the outbound catalog, which the scanner doesn't detect for this post shape), `applyDeviceStates` injects it once. No new pure unit to test (reuses the tested query/parse path; the only new logic is HTML + a trivial enum→label map); verified by compile. Bars reuse existing `crash-dist-*` CSS.
- **Per-issue trend mini-chart (T3.1) — DONE (2026-06-12).** One new DAILY `errorCountMetricSet` query grouped by `issueId` (`fetchIssueTrends` in `play-reporting-metrics.ts`; pure shaper `buildTrends` extracted to vscode-free `play-reporting-trends.ts`, 3 `node:test` cases). `getIssueTrends` wrapper mirrors `getIssueFilterIndex`. Lazy: `handleCrashlyticsTrends` renders each series to an SVG via the shared `renderSparkline` (reused from T1.1) and posts a `{shortIssueId → svg}` map (`crashlyticsTrends`, outbound catalog regenerated) triggered by `fetchCrashlyticsTrends` after the list renders (incoming catalog regenerated). Webview `applyCpTrends` injects each SVG into the row's `.cp-trend` slot by short id. `.cp-trend` CSS. The sparkline-module reuse from 6a paid off exactly as predicted.
