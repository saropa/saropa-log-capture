...
Phase 1–3, retirement, and **Unified Insight Model (Phase 2)** are complete. **One Insight panel**, one scroll, no tabs. Accordion sections: **Active Cases** (top 3 + View All), **Recurring errors** (top 5), **Frequently modified files** (collapsed), **Environment** (platforms, SDK versions, debug adapters; collapsed), **Performance** (when a log is open). **State A** (no log): Cases + Recurring + Hot files + Environment. **State B** (log selected): Performance (with scope label "Current log: &lt;filename&gt;", **compact hero**: sparkline from .perf.json when available + Errors/Warnings/Snapshot metrics), **Errors in this log** (top 3 from session fingerprints), **Recurring in this log** (filtered to errors that appear in the current session), Recurring, Cases (collapsed). **Full add-to-case:** "+" on recurring cards (global and in-this-log), errors-in-this-log, and hot files runs `addInsightItemToCase` (quick pick investigation → append line to notes → open Cases). **requestInsightData** → insightData (errors, statuses, hotFiles, platforms, sdkVersions, debugAdapters, recurringInThisLog, errorsInThisLog, errorsInThisLogTotal when currentFileUri set). **currentLogChanged** triggers requestPerformanceData and requestInsightData. L10n and docs updated. All planned items implemented.
