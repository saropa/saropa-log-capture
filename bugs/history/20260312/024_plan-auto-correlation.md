...
**UI:** Timeline panel shows ▶ badge on correlated event rows; click highlights all events in the same correlation. Log viewer receives setCorrelationByLineIndex when loading a session log and shows ▶ on lines that are part of a correlation. New **Correlations** sidebar view lists correlations for the last opened timeline with "Jump to event" (open log at line or sidecar file).
