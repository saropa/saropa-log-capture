...
Phase 1 (done): Spawn `adb logcat` as a child process alongside the debug session, stream lines through `logSession.appendLine()` with category `'logcat'`, buffer for `.logcat.log` sidecar at session end. PID filtering from DAP process event. Auto-suggest prompt when Dart/Flutter debug session starts with adb on PATH.
...
- `src/modules/integrations/adb-logcat-suggest.ts` — auto-suggest prompt on Dart/Flutter session start
- `src/modules/config/config-types-integrations.ts` — `IntegrationAdbLogcatConfig`

## Settings

Enable: add `"adbLogcat"` to `saropaLogCapture.integrations.adapters`, or accept the auto-prompt when debugging Dart/Flutter with adb on PATH.
