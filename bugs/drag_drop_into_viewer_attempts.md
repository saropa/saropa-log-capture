...
**Required before attempt 3:** the user picks a direction (remove vs. add a tree drop surface), OR provides the attempt-2 diagnostic outcome (overlay? toast? nothing?) so we know whether the drop even reaches the iframe.
