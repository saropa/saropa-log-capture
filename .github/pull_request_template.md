...
- [ ] If you added or changed **extension → webview** `postMessage` payloads (`type` field from host code), run `npm run generate:host-outbound-catalog` and commit `doc/internal/webview-outbound-message-types.md` (see `scripts/modules/generate/webview-host-outbound-catalog.mjs` for scan rules)
